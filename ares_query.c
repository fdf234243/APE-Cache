...
void ares_query_hash(ares_channel channel, const char *name, const char* hash_url, int dnsclass,
                int type, ares_callback callback, void *arg)
{
  struct qquery *qquery;
  unsigned char *qbuf;
  ...
  /* Compose the query. */
  rd = !(channel->flags & ARES_FLAG_NORECURSE);
  status = ares_create_query_hash(name, hash_url, dnsclass, type, channel->next_id, rd, &qbuf,
                             &qlen, (channel->flags & ARES_FLAG_EDNS) ? channel->ednspsz : 0);
  if (status != ARES_SUCCESS)
  {
    if (qbuf != NULL) ares_free(qbuf);
